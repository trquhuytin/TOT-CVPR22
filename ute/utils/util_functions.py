...
def save_params(opt, path_to_write):

    with open(os.path.join(path_to_write, 'params.txt'), 'w') as f:
...
def save_video_assignments(name, segmentation, phase, root_dir):
    #("Called with Phase: {}".format(phase))
    save_dir = os.path.join(root_dir, "video_segmentation", phase)
    
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    
    with open(save_dir + "/" + name + '.pkl', 'wb') as f:
            pickle.dump(segmentation, f, pickle.HIGHEST_PROTOCOL)
