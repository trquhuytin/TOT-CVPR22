...
class Visual(object):
    def __init__(self, opt, mode='pca', dim=2, reduce=None, save=False, svg=False, saved_dots=''):
    ...
    def plot(self, iter=0, show=True, prefix=''):
        ...
        if self._save:
            ...
            dir_check(join(opt.tensorboard_dir, 'plots'))
            dir_check(join(opt.tensorboard_dir, 'plots', opt.subaction))
            # name = ['iter%d_' % self._counter, 'gt_'][gt_plot]
            name = prefix + '%s_%s_' % (opt.subaction,  opt.model_name)
            folder_name = opt.log_str
            dir_check(join(opt.tensorboard_dir, 'plots', opt.subaction, folder_name))
            ...
            plt.savefig(join(opt.tensorboard_dir, 'plots', opt.subaction,
                             folder_name, name), transparent=True, dpi=300)
            np.savetxt(join( opt.tensorboard_dir, 'plots', opt.subaction,
