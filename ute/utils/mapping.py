...
class GroundTruth:
    ...
    @timing
    def load_gt(self, opt):
        self.gt = self.load_obj('gt%d%s' % (self.frequency, opt.gr_lev), opt)
        self.order = self.load_obj('order%d%s' % (self.frequency, opt.gr_lev), opt)
        ...
        if self.gt is None or self.order is None:
            ...
            self.save_obj(self.gt, 'gt%d%s' % (self.frequency, opt.gr_lev), opt)
            self.save_obj(self.order, 'order%d%s' % (self.frequency, opt.gr_lev), opt)
        else:
    ...
    def load_mapping(self, opt):
        logger.debug('load or create mapping')
        self.create_mapping(opt)
        self.load_gt(opt)
